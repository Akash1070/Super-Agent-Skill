...
[![Tools Count](https://img.shields.io/badge/Tools-62%20Active%20Tools-orange.svg?style=for-the-badge)](#-the-62-tool-suite-across-28-modules)
...
## 📦 The 62-Tool Suite Across 28 Modules

| Module | Purpose | Tools Provided |
| :--- | :--- | :--- |
| **1. AI Workflow & Multi-Agent** | LangGraph task chains, AutoGen crews, OpenTelemetry & Developer Resumes | `generate_ai_workflow_orchestration`, `generate_observability_blueprint`, `generate_resume_portfolio_blueprint` |
| **2. Federated Memory Sync** | Global community memory sync across 100+ developers | `sync_collective_community_memory`, `get_community_wisdom_report` |
| **3. Stage Advisor Engine** | Project stage diagnostic & step-by-step developer action blueprint | `identify_project_stage_and_roadmap`, `generate_developer_value_walkthrough` |
| **4. Live GitHub Scout** | Real-time GitHub search & 24h auto-updater | `scout_and_update_skill_library`, `get_latest_trending_repos` |
| **5. Gold Memory Database** | Persistent habit, mistake autopsy & memory search | `log_user_preference`, `log_mistake_autopsy`, `get_agent_memory`, `get_live_memory_dashboard`, `generate_in_ide_improvement_notification`, `search_agent_memory` |
| **6. Auth & Architecture** | Modern auth & backend stack recommendations | `get_auth_architecture_blueprint`, `get_backend_stack_recommendation` |
| **7. Prompt Intelligence** | Code comprehension & system prompt optimization | `get_code_comprehension_strategy`, `optimize_agent_prompt` |
| **8. UI/UX & Mobile Design** | Design system generator & mobile layout blueprints | `get_design_system_recommendation`, `get_ui_component_blueprint`, `get_mobile_app_blueprint`, `audit_ui_design_aesthetic` |
| **9. DevOps & Deploy** | Multi-stage Docker generator & failure diagnosis | `generate_docker_setup`, `get_deployment_checklist`, `diagnose_deployment_issue` |
| **10. Code Security** | OWASP security auditor & code hardening | `audit_security_vulnerabilities`, `generate_security_hardening` |
| **11. Karpathy Guidelines** | Surgical scope & goal verification guidelines | `get_karpathy_rules`, `evaluate_coding_plan`, `generate_verification_checklist` |
| **12. Performance & Vitals** | Web Vitals audit & Redis caching strategies | `audit_performance_bottlenecks`, `generate_caching_strategy` |
| **13. Testing & QA** | Playwright / Vitest / PyTest test suite generator | `generate_test_suite_blueprint` |
| **14. API Contracts** | Zod schema & TypeScript type contract generator | `generate_typesafe_api_contract` |
| **15. GitHub CI/CD** | Production GitHub Actions workflow generator | `generate_github_actions_workflow`, `generate_autonomous_pr_fixer` |
| **16. Code Refactoring** | Ponytail codebase consolidation & dead code detection | `modernize_legacy_code`, `detect_dead_code_and_bloat`, `ponytail_codebase_modernizer` |
| **17. SEO Metadata** | Social meta tags & JSON-LD schema generator | `generate_seo_metadata_blueprint` |
| **18. Growth & Marketing** | Alex Hormozi $100M offers, positioning & ad copy | `generate_marketing_copy_blueprint`, `generate_hormozi_offer_blueprint`, `generate_brand_positioning_matrix`, `generate_ad_copy_blueprint`, `generate_ai_image_prompt_blueprint` |
| **19. Monetization & Pricing** | SaaS pricing tiers & unit economics (LTV/CAC) | `generate_pricing_tier_blueprint`, `generate_unit_economics_calculator` |
| **20. Legal & Compliance** | GDPR/CCPA Privacy Policy & Terms of Service | `generate_privacy_terms_blueprint` |
| **21. Product Analytics** | Event telemetry tracking & funnel optimization | `generate_telemetry_funnel_blueprint` |
| **22. SEO + AEO + GEO** | Search, Perplexity AEO & LLM GEO `llms.txt` discovery | `generate_seo_aeo_geo_blueprint`, `generate_llms_txt_blueprint`, `audit_seo_aeo_geo_readiness`, `generate_backlink_and_traffic_strategy` |
| **23. RAG & Vector DBs** | ChromaDB / Qdrant RAG & document parsing pipelines | `generate_rag_vector_blueprint`, `generate_document_processing_pipeline` |
| **24. Stealth Web Scraping** | Playwright headless browser automation & stealth scraping | `generate_web_scraping_blueprint` |
| **25. Data Visualization** | Recharts / Chart.js / Tremor analytics dashboard components | `generate_dataviz_dashboard_blueprint` |
| **26. Multimodal AI & Media** | Whisper audio transcription, Vision LLMs & Synthetic Data | `generate_multimodal_ai_blueprint` |
| **27. Web, CMS & E-Commerce** | Stripe checkout, Headless CMS (Payload) & WebAssembly | `generate_web_domain_blueprint` |
| **28. Fintech & Trading** | Quantitative trading execution loops & Plaid banking APIs | `generate_fintech_trading_blueprint` |
